  ...
  > = useQuery({
    ...
    queryFn: async () => {
      const response = await axios.get(
        `${SHARED_SERVER_URL}/participant/profile/${participantId}/${session?.user.userId}`
      );
      return response.data;
    },
    enabled: !!participantId && status === "authenticated",
  });
                        ...
                        <div className="bg-[#3A3B3C] py-1 px-3 flex space-x-1 rounded-sm items-center border border-slate-600">
                          ...
                          <span className="text-white text-[0.85rem]">
                            {format(
                              new Date(
                                getParticipantProfile.data
                                  ?.conversation_started_at as Date
                              ) as Date,
                              "MM/dd/yyyy"
                            )}
                          </span>
          ...
          <div className="w-full flex flex-col space-y-3">
            ...
            <div>
              ...
              >
                <small className="text-white text-[0.8rem] text-center whitespace-pre-wrap">
                  {format(
                    new Date(participant_details?.createdAt as Date),
                    "MM/dd/yyyy"
                  )}
                </small>
              </div>
